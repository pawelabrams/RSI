...
if __name__ == "__main__":
    ...
    for y in range(block_count):
        ...
        qi = Queue()
        qo = Queue()
        dg = Process(target=obsluz, args=(url, qi, qo))
        dg.start()
        srv_array.append([dg, qi, qo])
        if y % lensrv == 0:
            port_number += 1
    ...
    machine_no = 0
    for s in srv_array:
        s[1].put(numbers[machine_no * block_size:(machine_no + 1) * block_size])
        machine_no += 1
    ...
    one_last_reduce = []
    for element in srv_array:
        one_last_reduce.append(element[2].get())
